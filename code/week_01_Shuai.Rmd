...
2. Second Thought (criteria for subsetting data ):
...
```{r include=FALSE}
base <- subset(base, Loan_Percentage <= 1 & Male != "." & Married != "." & Credit_History != 666)
...
(Code is hid from this paper, but avaiable based on request)

__Findings from race and gender: __

  1. THe propostion of Black and Hispanic applications account for 10% and 5% of all applications seperately. 
  2. THe propostion of Male and Female applications account for 19% and 81% of all applications seperately. 
...
__Findings from race and decision: __
  ...
  1. Among all appliction the approved application from Black counts for 6.6%. The approval rate for applications from Black was __66.7%__.
  2. Among all appliction the approved application from Hispanic counts for 4.2%. The approval rate for applications from Hispanic was __77.9%__.
  3. Among all appliction the approved application from Non-Black/Hispanic counts for 77.1%.The approval rate for applications from Non-Black\Hispanic was __91.0%__.
...
__Findings from race and credit history: __
  ...
  1. Among all appliction that the Black applicant with good credit history counts for __7.2%__. 72.3% Black applicants had good credite history.
  2. Among all appliction that the Hispanic applicat with good credit history counts for __4.6%__. 86.5% Hispanic applicants had good credite history.
  3. Among all appliction that the Non-Black/Hispanic applicat with good credit history counts for __79.6%__. 93.9% Non-Black/Hispanic applicants had good credite history.
...
__Findings from race, credit history, and decision: __
  ...
  1. Among all applict who were with "Good" credit history, the approval rate was __"Not Hispan/Black" > "Black" > "Hispan"__.
  2. Among all applict who were with "Not Good" credit history, the approval rate was __"Not Hispan/Black" > "Hispan" > "Black"__.
...
__Findings from distribution of loan rate by races and genders: __
  ...
  1. Fig 1 and Fig 2 below indicate that, regardless the race and gender, most of applicats applies for the loan amout around 75% to 90% of the purchase price.
...
```{r warning=FALSE}
LogitModel = glm(Approve ~ Other_Obligations + Black + Hispan, data = base , family = "binomial")
summary(LogitModel)
```
The signals of the model suggest that:  
  1.	Increases in Other Obligation diminish the likelihood of approval of the loan, which makes sense.  
  2.	Being Black or Hispanic diminish your likelihood of approval of the loan, which doesn’t make sense (your ethnicity doesn’t interfere to your capacity to pay a loan), but was expected.  

__(B) Generate Odds Ratios__
```{r warning=FALSE}
exp(coef(LogitModel))
```
The Odd ratios suggest that:  
...
```{r warning=FALSE}
prototype_black <- data.frame(Other_Obligations=mean(base$Other_Obligations),Black = 1, Hispan = 0)
prototype_black <-prototype_black %>% mutate(Black = as.factor(Black),
                                             Hispan = as.factor(Hispan))
# levels(prototype_black$Black) <- "Black"
# levels(prototype_black$Hispan) <- NA

prototype_hisp <- data.frame(Other_Obligations=mean(base$Other_Obligations),Black = 0, Hispan = 1)
prototype_hisp <-prototype_hisp %>% mutate(Black = as.factor(Black),
                                             Hispan = as.factor(Hispan))
# levels(prototype_hisp$Black) <- NA
# levels(prototype_hisp$Hispan) <- "Hispan"

prototype_white <- data.frame(Other_Obligations=mean(base$Other_Obligations),Black = 0, Hispan = 0)
prototype_white <-prototype_white %>% mutate(Black = as.factor(Black),
                                             Hispan = as.factor(Hispan))
# levels(prototype_white$Black) <- NA
# levels(prototype_white$Hispan) <- NA
```
...
__(D-1) Predict probabilities for prototypical individuals - Black__
```{r warning=FALSE}
prototype_black
```

__(D-2) Predict probabilities for prototypical individuals - Hispan__
```{r warning=FALSE}
prototype_hisp
```

__(D-3) Predict probabilities for prototypical individuals - White__
```{r warning=FALSE}
prototype_white
```

We can see that White man with % of other obligations has `r round(prototype_white$predictedprob,digits = 2)`% of chances to be approved for a loan. At the same situation, black people have, for the same situation only `r round(prototype_black$predictedprob,digits = 2)`% of chances to be approved for a loan and Hispanics have only `r round(prototype_hisp$predictedprob,digits = 2)`% of chances to be approved for a loan.

__(E) Estimate Probit Model__
```{r warning=FALSE}
ProbitModel = glm(Approve ~ Other_Obligations + Black + Hispan, data = base, 
...
__(F-1) Predict probabilities for prototypical individuals - Black__
```{r warning=FALSE}
prototype_black
```

__(F-2) Predict probabilities for prototypical individuals - Hispan__
```{r warning=FALSE}
prototype_hisp
```

__(F-3) Predict probabilities for prototypical individuals - White__
```{r warning=FALSE}
prototype_white
```
We can see that White man with `r round(prototype_hisp$Other_Obligations,digits = 2)`% of other obligations has `r round(prototype_white$predictedprob,digits = 2)`% of chances to be approved for a loan. At the same situation, black people have, for the same situation only `r round(prototype_black$predictedprob,digits = 2)`% of chances to be approved for a loan and Hispanics have only `r round(prototype_hisp$predictedprob,digits = 2)`% of chances to be approved for a loan.  
...
The signals of the model suggest that:  
  1.	Increases in Other Obligation diminish the likelihood of approval of the loan, which makes sense.  
  2.	Being woman diminish your likelihood of approval of the loan, which doesn’t make sense (your gender doesn’t interfere to your capacity to pay a loan), but was expected. 
__(B) Generate Odds Ratios__
```{r warning=FALSE}
exp(coef(LogitModel))
```
